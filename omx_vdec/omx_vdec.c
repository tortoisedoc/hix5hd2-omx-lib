       ...
       OMX_IN OMX_U8* p_handle)
{
    ...
    pvdec_buf->eBufferType  = OMX_USE_NATIVE_TYPE;
       ...
       OMX_IN OMX_U8 *buffer)
{
	...
	pvdec_buf->buffer_len  = bytes;
	pvdec_buf->client_data = (OMX_U32)pomx_buf;
    pvdec_buf->eBufferType = OMX_USE_TYPE;
       ...
       OMX_IN OMX_U32 bytes)
{
	...
	pvdec_buf->client_data = (OMX_U32)pomx_buf;
    pvdec_buf->eBufferType = OMX_ALLOCATE_TYPE;
       ...
       OMX_IN OMX_BUFFERHEADERTYPE *omx_bufhdr)
{
	...
	if (OMX_ALLOCATE_TYPE == puser_buf->eBufferType)
	{
		free_contigous_buffer(puser_buf);
	}
    else if (OMX_USE_NATIVE_TYPE == puser_buf->eBufferType)
	{
       ...
       OMX_U32  width, OMX_U32  height)
{
    ...
    port_priv->port_pro.buffer_size = FRAME_SIZE(align_width, height);
...
static OMX_ERRORTYPE get_supported_profile_level(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_VIDEO_PARAM_PROFILELEVELTYPE *profileLevelType)
{
	...
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_VC1, OMX_MAX_STRINGNAME_SIZE))
	{
	...
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_DIVX3, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = divx3_profile_level_list;
		max_num = COUNTOF(divx3_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_VP8, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = vp8_profile_level_list;
		max_num = COUNTOF(vp8_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_VP6, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = vp6_profile_level_list;
		max_num = COUNTOF(vp6_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_WMV, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = wmv_profile_level_list;
		max_num = COUNTOF(wmv_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_AVS, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = avs_profile_level_list;
		max_num = COUNTOF(avs_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_SORENSON, OMX_MAX_STRINGNAME_SIZE))
	{
		pinfo = sorenson_profile_level_list;
		max_num = COUNTOF(sorenson_profile_level_list);
	}
	else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_COMPONENTROLES_RV, OMX_MAX_STRINGNAME_SIZE))
	{
       ...
       OMX_IN OMX_PTR param_data)
{
	...
	switch (param)
	{
		case OMX_IndexParamPortDefinition:
		{
			...
			ret = update_picture_info(pcomp_priv, portDefn->format.video.nFrameWidth, portDefn->format.video.nFrameHeight);
			if (ret != OMX_ErrorNone)
			{
		...
		case OMX_IndexParamVideoPortFormat:
		{
			...
			if (INPUT_PORT_INDEX == portFmt->nPortIndex)
			{
				...
				else
				{
                    ...
                    pcomp_priv->m_dec_fmt = (OMX_VIDEO_CODINGTYPE)codec_trans_list[i].compress_fmt;
                    
                    if (pcomp_priv->m_dec_fmt != OMX_VIDEO_CodingRV 
                     && pcomp_priv->m_dec_fmt != ((OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingVP6))
                    {
                        pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd = codec_trans_list[i].codec_type;
                    }
                }
			}
			else if (OUTPUT_PORT_INDEX == portFmt->nPortIndex)
...
OMX_ERRORTYPE component_init(OMX_HANDLETYPE phandle, OMX_STRING comp_name)
{
	...
	if (strncmp(comp_name, OMX_VDEC_COMPONENT_NAME, OMX_MAX_STRINGNAME_SIZE) != 0)
	{
